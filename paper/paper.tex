...
measured gaze duration and expected true duration under a variety of changing scenarios. 
These scenarios are variations in the underlying error indicated by the eye tracking 
...
We include these biased calibration dataset as they are consistent with both our 
...
\caption{Measured Versus Expected Gaze Duration. \\\hspace{\textwidth} 
MREC ad appearing on screen for 1s with 50px Mean Error in Fixation}
...
tendency for the measurement to be an accurate estimation of true gaze duraiton, while 
at higher values it tends toward an under-estimation of the true gaze duration. 
Surprisingly the results using the biased calibration data seem to mitigate this effect 
somewhat, but the overall pattern remains consistent.
